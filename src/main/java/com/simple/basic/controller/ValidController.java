...
public class ValidController {
	...
	public String viewForm(@Valid @ModelAttribute("vo") ValidVO vo, Errors errors, Model model) {
		...
		if(errors.hasErrors()) {
			...
			for(FieldError err : list ) {
				...
				//System.out.println(err.getField()); // 에러가 난 필드명 key
				//System.out.println(err.getDefaultMessage()); //메시지 출력 value
				//System.out.println(err.isBindingFailure()); //유효성검사에 의해서 err 라면 false , 아니면 true
	...
	public String quizForm(@Valid @ModelAttribute("vo") MemberVO vo, Errors errors , Model model) {
		...
		if(errors.hasErrors()) {
			List<FieldError> elist = errors.getFieldErrors();

			for(FieldError er : elist) {

				if(er.isBindingFailure()) {
					model.addAttribute("valid_" + er.getField(), "잘못 입력!!");
				} else {
					model.addAttribute("valid_" + er.getField(), er.getDefaultMessage());
